...
def get_stats_json_paths(results_folder, sub_folder="val"):
...
def find_best_stat(stats_results, min_train_time):
    best_stats = []
    ...
    for stat in stats_results:
        ...
        best_stats.append({'model': model_name, 'measure': measurement, 'vn': with_vn, 'dataset': data_set_name, 'stat': best_stat})
...
def generate_table(data_set_name, stats) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        measure + ' (after VN)': [x[1] for x in vn_stats] + [None] * 2
    })
